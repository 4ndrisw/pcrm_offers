...
$aColumns = hooks()->apply_filters('offers_relation_table_sql_columns', $aColumns);
...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '<a href="' . admin_url('offers/list_offers/' . $aRow['id']) . '">' . format_offer_number($aRow['id']) . '</a>';

    $numberOutput .= '<div class="row-options">';

    $numberOutput .= '<a href="' . site_url('offer/' . $aRow['id'] . '/' . $aRow['hash']) . '" target="_blank">' . _l('view') . '</a>';
    if (has_permission('offers', '', 'edit')) {
        $numberOutput .= ' | <a href="' . admin_url('offers/offer/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }
    $numberOutput .= '</div>';

    $row[] = $numberOutput;

    $row[] = '<a href="' . admin_url('offers/list_offers/' . $aRow['id']) . '">' . $aRow['subject'] . '</a>';
